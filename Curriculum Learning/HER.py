...
class DQN(object):
    ...
    def learn(self):
        ...
        if self.test:
            ...
            _, _, mini_batch = self.memory.sample(self.opt["mini_batch_size"])
    ...
    def store(self, ob, action, new_ob, reward, done,goal, it=None):
...
if __name__ == '__main__':
    ...
    env, config, outdir, logger = init('./config_curriculum.yaml', "HER")
    ...
    for i in range(episode_count):
        ...
        while True:
            ...
            if done:
                ...
                logger.direct_write("x-coordinate", new_ob[0][0], i)
                logger.direct_write("y-coordinate", new_ob[0][1], i)
                mean += rsum
                rsum = 0

                if not agent.test:
                   sample_goal=new_ob
                   for tr in episode_exp:
                        ob_old,a_old,new_ob_old=tr
                        done=(new_ob_old==sample_goal).all()
                        reward=1.0 if done else -0.1
                        agent.store(ob_old, a_old, new_ob_old, reward, done,sample_goal)
